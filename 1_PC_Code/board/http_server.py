    ...
    def do_GET(self):
        if self.path == "/" or self.path == "/index.html":
            ...
            self._ok({
                ...
                "camera_active": snap.camera_active,
                "camera_alert_level": snap.camera_alert_level,
                "camera_alert_msg": snap.camera_alert_msg,
                "camera_vehicles": snap.camera_vehicles,
            })
            ...
            lat, lon = float(body.get("lat", 0)), float(body.get("lon", 0))
            name = body.get("name", "收藏地点")
            ...
            from ai_alert import start_ai_monitor
            start_ai_monitor()
            self._ok({"status": "started"})
        elif self.path == "/camera/status":
            from camera_manager import camera
            self._ok({"code": 0, "data": camera.get_status()})
        elif self.path == "/camera/scan":
            from camera_manager import camera
            self._ok({"code": 0, "data": {"devices": camera.scan()}})
        elif self.path == "/camera/start":
            from camera_manager import camera
            from ai_monitor import ai_monitor
            from ai_alert import handle_detection
            if not camera.open():
                self._ok({"code": 1001, "msg": "摄像头打开失败"})
                return
            with state as s:
                s.camera_active = True
            ai_monitor.start(callback=handle_detection)
            self._ok({"code": 0, "msg": "camera_started"})
            ...
            self._ok({"code": 0, "msg": "camera_stopped"})
        elif self.path == "/camera/frame":
            from camera_manager import camera
            jpg = camera.get_latest_jpeg()
            if jpg:
                self.send_response(200)
                ...
                self._ok({"code": 1005, "msg": "no frame available"})
        elif self.path == "/ping":
