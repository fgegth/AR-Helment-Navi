...
# 类别名 → ID 反向映射 (标签文件加载后NPU输出类别名而非ID)
_NAME_TO_ID = {v: k for k, v in VEHICLE_CLASSES.items()}

def _parse_output(output: str, img_w: int = INPUT_SIZE, img_h: int = INPUT_SIZE) -> list:
    """解析 YOLOv5 输出 → Detection 列表 (兼容两种格式)"""
    detections = []
    for line in output.strip().split("\n"):
        ...
        # 格式1: "cls_id conf x1 y1 x2 y2" (无标签文件时)
        parts = line.split()
        if len(parts) >= 6:
            try:
                cls_id = int(parts[0])
        ...
        # 格式2: "class_name @ (x1 y1 x2 y2) confidence" (有标签文件时)
        if "@" in line:
            try:
                name_part, rest = line.split("@", 1)
                name = name_part.strip()
                # 提取坐标: (x1 y1 x2 y2)
                coords_str = rest.split(")")[0].split("(")[-1].strip()
                conf_str = rest.split(")")[-1].strip()
                x1, y1, x2, y2 = [float(c) for c in coords_str.split()]
                conf = float(conf_str)
                cls_id = _NAME_TO_ID.get(name)
                if cls_id is not None and conf > 0.5:
                    detections.append(Detection(cls_id, conf, x1, y1, x2, y2, img_w, img_h))
