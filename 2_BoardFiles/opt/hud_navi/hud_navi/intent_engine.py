...
INTENT_RULES = [
    ...
    # 取消 → 必须在导航前面! 否则"取消导航去公司"被误判为navigate
    (r'(取消|停止|关闭|结束).*(导航|路线|行程)', "cancel", False),
    (r'^别.*导航', "cancel", False),
    # 查询
...
HARDCODED_PLACES = {
    ...
    "合肥逍遥津": (31.8650, 117.3010), "合肥逍遥津公园": (31.8650, 117.3010),
    # 常见通用目的地（无固定坐标，由 frequent_places 或调用方解析）
    "回家": None, "家": None,
}
