...
class VoiceAlert:
    ...
    def _pre_cache(self):
        """预缓存: 仅生成Piper TTS缓存文件, 不播放"""
        try:
            from hud_tts import get_tts
            tts = get_tts()
            if not tts.is_ready():
                return
        except Exception:
        ...
        for text in phrases:
            try:
                tts.synthesize(text)  # 仅生成WAV, 不播放
        ...
        while self._running:
            try:
                ...
                with audio_lock:  # 播放时锁Codec, 防止录音冲突
                    # 1. Piper TTS 离线真人声 (ARM64原生, 无需网络, 首选)
    ...
    def _try_beep(self, text: str) -> bool:
        ...
        for _ in range(beeps):
            try:
                ...
                # BusyBox aplay -d 不接受小数, 移除-d让数据长度决定时长
                for dev in ("plughw:2,0", "plughw:0,0", "default"):
                    try:
                        subprocess.run(["aplay", "-q", "-D", dev, "-t", "raw", "-f", "S16_LE",
                            "-r", "16000", "-c", "1"], input=tone, timeout=1)
                        break
                    except Exception:
                        continue
            except: pass
