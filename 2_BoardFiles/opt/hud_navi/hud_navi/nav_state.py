...
class NavState:
    ...
    # ---- 传感器融合 (sensor_fusion.py) ----
    fused_position: Optional[Tuple[float, float]] = None  # EKF融合位置
    fused_heading: float = 0.0         # EKF融合航向
    fusion_confidence: float = 0.0     # 融合置信度 0.0~1.0
    fusion_active: bool = False        # 融合引擎是否运行
    dead_reckoning_active: bool = False  # 纯推测导航模式
    dead_reckoning_remaining: float = 0.0  # 推测导航剩余时间(秒)
...
class SharedState:
    ...
    def get_snapshot(self) -> NavState:
        ...
        with self:
            ...
            snapshot.fused_position = self._state.fused_position
            snapshot.fused_heading = self._state.fused_heading
            snapshot.fusion_confidence = self._state.fusion_confidence
            snapshot.fusion_active = self._state.fusion_active
            snapshot.dead_reckoning_active = self._state.dead_reckoning_active
            snapshot.dead_reckoning_remaining = self._state.dead_reckoning_remaining
