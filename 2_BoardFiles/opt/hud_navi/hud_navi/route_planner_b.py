...
class RoutePlanner:
    ...
    # ================================================================
    # 几何工具: 点到线段距离 (共享, 避免重复代码)
    # ================================================================

    @staticmethod
    def _point_to_segment_m(point, a, b):
        """点到线段ab的近似距离(米), 使用平面投影"""
        lat0, lon0 = a
        def _to_m(p):
            return ((p[0] - lat0) * 111320.0,
                    (p[1] - lon0) * 111320.0 * math.cos(math.radians(lat0)))
        px, py = _to_m(point); ax, ay = _to_m(a); bx, by = _to_m(b)
        abx, aby = bx - ax, by - ay
    ...
    def _point_to_polyline_distance(
        ...
        for i in range(len(polyline) - 1):
            d = self._point_to_segment_m(point, polyline[i], polyline[i + 1])
    ...
    def _distance_along_step(
        ...
        for i in range(len(coords) - 1):
            d = self._point_to_segment_m(current_pos, coords[i], coords[i + 1])
            if d < best_dist:
                best_dist = d
                best_seg_idx = i
