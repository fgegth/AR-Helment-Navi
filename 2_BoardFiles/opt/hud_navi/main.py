...
class NavigationEngine:
    ...
    def tick(self, voice_alert: VoiceAlert):
        ...
        if snap.is_navigating and snap.destination:
            ...
            if need_plan:
                self._plan_route(current_pos, snap.destination, voice_alert)
    ...
    def _plan_route(self, origin, destination, voice_alert=None):
        ...
        if result:
            ...
            # 开始导航语音反馈
            if voice_alert:
                voice_alert.speak("导航开始", force=True)
        else:
            with state as s:
...
def check_system_status():
    ...
    elif not snap.gps_quality["signal_weak"]:
        # GPS信号恢复, 清除过期警告
        with state as s:
            if "GPS信号弱" in s.error_message:
                s.error_message = s.error_message.replace("GPS信号弱", "").replace(" |  | ", " | ").strip(" |")
...
def main():
    ...
    def handle_voice_command(cmd_text):
        ...
        if i == "navigate" and result.get("lat") and result.get("lon"):
            ...
            with state as s:
                s.is_navigating = False; s.is_arrived = False; s.destination = None; s.route = []
            logger.info("语音取消导航")
        elif i == "status":
            snap = state.get_snapshot()
    ...
    # 摄像头AI — 独立守护进程 (guard.sh启动camera_daemon.py)
    try:
    ...
    # ---- 5.5. GPS 初始状态 (非阻塞, tick会自动处理无GPS情况) ----
    logger.info(f"GPS冷启动约{GPS_COLD_START_SEC}秒, 导航引擎立即开始工作...")
    with state as s:
        s.error_message = "GPS定位中..."
    _gps_first_fix = False  # 首次定位标志
    ...
    try:
        ...
        while not _shutdown_flag:
            ...
            try:
                ...
                if _os.path.exists('/tmp/camera_alert.json'):
                    with open('/tmp/camera_alert.json') as f:
                        alert = _json.load(f)
                    with state as s:
                        s.camera_alert_level = alert.get('level', 0)
                        s.camera_alert_msg = alert.get('msg', '')
                        s.camera_vehicles = alert.get('vehicles', 0)
            except Exception:
                pass
            ...
            # 首次GPS定位成功检测 (非阻塞, 冷启动约35秒)
            snap = state.get_snapshot()
            if not _gps_first_fix and snap.current_position is not None:
                _gps_first_fix = True
                logger.info(f"GPS定位成功! ({snap.current_position})")
                with state as s:
                    if s.error_message == "GPS定位中...":
                        s.error_message = ""
